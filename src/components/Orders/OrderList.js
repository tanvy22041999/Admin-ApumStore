            ...
            <CCard>
                  ...
                  <div className="col-6 col-md-3">
                    <div className="card bg-primary">
                      <div className="p-2">
                        ...
                        <select className="form-control mt-2" value={filter.paymentMethod} name="paymentMethod" onChange={this.handleChangeFilter}>
              ...
              <CCardBody>
                <CDataTable
                  ...
                  scopedSlots = {{
                    'Date of create': (item) => (
                      <td>{new Date(item.createTimeStamp).toLocaleDateString("vn-VN")}</td>
                    ),
                    'user': (item) => (
                      <td className="text-center">
                        {item.user && <><div className="c-avatar">
                          <CImg
                            src={item.user.image ? item.user.image.publicUrl : INITIAL_IMAGE}
                            className="c-avatar-img"
                            alt={item.user.id}
                          />
                        </div>
                        <p className="mb-0">{item.user.firstname} {item.user.lastname}</p>
                        <p className="mb-0">({item.shippingPhonenumber})</p></>}
                      </td>
                    ),
                    'Payment Method': (item) => (
                      <td>{item.paymentMethod=== "local" ? '(COD) Tiền mặt' : 'Paypal'}</td>
                    ),
                    'Payment Status': (item) => (
                    ...
                    (item)=>(
                      <td>
                        <Button id={`UncontrolledPopover${item.id}`} type="button">
                          <i className="fa fa-chevron-circle-down"></i>
                        </Button>
                        <UncontrolledPopover trigger="focus" placement="bottom" target={`UncontrolledPopover${item.id}`}>
