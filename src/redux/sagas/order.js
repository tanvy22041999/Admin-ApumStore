...
function* handleUpdate({ payload }) {
  try {
    ...
    if(payload.data.status === "0"){
      socket.emit('orderChangeStatus', { status: 0, user: data.order.user.toString(), order: data.order.id });
      yield put(NotificationActions.onCreate({
        user: data.order.user,
        type: 0,
        link: data.order.id,
        name : `Đơn hàng ${data.order.id} đang trong quá trình vận chuyển`,
        image : data.order.order_list[0].product.bigimage,
        content :  `${data.order.id} vừa nhập kho vận chuyển`
      }))
    }
    else if(payload.data.status === "1"){
      socket.emit('orderChangeStatus', { status: 1, user: data.order.user.toString(), order: data.order.id });
      yield put(NotificationActions.onCreate({
        user: data.order.user,
        type: 0,
        link: data.order.id,
        name : `Đơn hàng ${data.order.id} đã vận chuyển thành công`,
        image : data.order.order_list[0].product.bigimage,
