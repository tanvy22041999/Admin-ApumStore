  ...
  try {
    ...
    if(payload.data.money){
      yield put(NotificationActions.onCreate({
        ...
        content :  `${data.installment.id} đã được thanh toán tại cửa hàng với số tiền là ${payload.data.money}`
      }))
    }
    else if(data.installment.status > -1){
      socket.emit('installmentChangeStatus', { status: 0, user: data.installment.user.toString(), installment: data.installment.id });
      yield put(NotificationActions.onCreate({
        user: data.installment.user,
        type: 2,
        link: data.installment.id,
        name : `Phiếu trả góp ${data.installment.id} vừa được duyệt`,
        image : detailResult.data.installment.product.id.bigimage.id,
        content :  `${data.installment.id} đã được duyệt thành công. Vui lòng kiểm tra thông tin trong Trang trả góp của bạn`
      }))
    }
  /* Notification */
  } catch (error) {
